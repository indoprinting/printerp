$(document).ready(function(e) {
    ...
    $(document).on('change', '#punit', function() {
        ...
        if (item.units && unit != reitems[item_id].row.base_unit) {
            $.each(item.units, function() {
                if (this.id == unit) {
                    $('#pprice')
                        .val(formatDecimal((parseFloat(item.row.real_unit_price) + parseFloat(aprice)) * unitToBaseQty(1, this), 4))
                        .change();
                }
            });
        } else {
            $('#pprice')
                .val(formatDecimal(parseFloat(item.row.real_unit_price) + parseFloat(aprice)))
                .change();
        }
    });
    ...
    $(document).on('change', '#poption', function() {
        ...
        var unit = $('#punit').val(),
            ...
            base_unit_price = item.row.real_unit_price;
        if (unit != reitems[item_id].row.base_unit) {
            $.each(reitems[item_id].units, function() {
                if (this.id == unit) {
                    base_unit_price = formatDecimal(parseFloat(item.row.real_unit_price) * unitToBaseQty(1, this), 4);
                    base_quantity = unitToBaseQty($('#pquantity').val(), this);
                }
            });
        }
                ...
                $.each(item.options, function() {
                    if (this.id == item_option) {
                        ...
                        if (this.price != 0 && this.price != '' && this.price != null) {
                            item_price = parseFloat(unit_price) + parseFloat(this.price) * parseFloat(base_quantity);
