...
class QueueManagementSystem {
  ...
  async getDisplayData (warehouse_id) {
    return new Promise((resolve, reject) => {
      resolve(this.qhttp.send('GET', site.base_url + 'qms/getDisplayData/' + warehouse_id));
    });
  }
  ...
  async sendDisplayResponse (ticket_id) {
    return new Promise((resolve, reject) => {
      ...
      resolve(this.qhttp.send('POST', site.base_url + 'qms/displayResponse/' + ticket_id, data));
    });
  }
  ...
  async sendReport (reportData = {}) {
    return new Promise((resolve, reject) => {
      let data = {};
      data[security.csrf_token_name] = security.csrf_hash;
      data = Object.assign(data, reportData); // Merge object.
      console.log('%cSent', 'color:lime');
      console.log(data);
      resolve(this.qhttp.send('POST', site.base_url + 'qms/sendReport', data));
    });
  }
}
...
class QueueNotify {
  constructor () {
    this.audio = {
      error: new Audio(`${site.assets}qms/audio/nasty-error-short.mp3`),
      success: new Audio(`${site.assets}qms/audio/when.mp3`),
      warning: new Audio(`${site.assets}qms/audio/system-fault.mp3`)
    };
  }
