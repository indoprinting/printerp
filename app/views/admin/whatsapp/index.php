...
<script>
  $(document).ready(function() {
    ...
    window.Table = $('#WhatsappTable').DataTable({
      ajax: {
        data: function(data) {
          data[security.csrf_token_name] = security.csrf_hash;
        },
        method: 'POST',
        url: site.base_url + 'whatsapp/getMessages?<?= $q ?>'
      },
      columnDefs: [{
          targets: 0,
          orderable: false,
      ...
      <ul class="btn-tasks">
        <li class="dropdown">
          ...
          <ul class="dropdown-menu dropdown-menu-right tasks-menus" role="menu" aria-labelledby="dLabel">
            <li>
              <a href="<?= admin_url('whatsapp/send'); ?>" id="add_trackingpod" data-toggle="modal"
                  data-backdrop="false" data-target="#myModal">
                <i class="fad fa-fw fa-plus-square"></i> Send Message
              </a>
            </li>
            <li class="divider"></li>
            <li>
              <a href="#" id="batchDelete">
                <i class="fad fa-fw fa-trash"></i> Delete Message
              </a>
            </li>
          </ul>
        </li>
  ...
  <div class="box-content">
    <div class="row">
      <div class="col-lg-12">
        ...
        <table id="WhatsappTable" class="table table-bordered table-condensed table-hover table-striped" cellpadding="0" cellspacing="0" borders="0" style="width:100%">
          <thead>
            <tr>
