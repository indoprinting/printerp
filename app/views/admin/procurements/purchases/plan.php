  ...
  $(document).ready(function() {
    Table = $('#Table').DataTable({
      ajax: {
        ...
        method: 'POST',
        url: site.base_url + 'procurements/purchases/getPurchasesPlan?<?= $q; ?>'
      },
      columnDefs: [{
          targets: 0,
          orderable: false,
...
<div class="box">
  <div class="box-header">
    <h2 class="blue"><i class="fa-fw fa fa-list-ol"></i>Purchases Plan<?= (isset($filter['start_date']) ? " From ({$filter['start_date']}) to ({$filter['end_date']})" : ''); ?></h2>
    <div class="box-icon">
      <ul class="btn-tasks">
        <li class="dropdown">
          ...
          <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
            <li>
              <a href="#" id="create_purchases">
                <i class="fad fa-fw fa-plus-circle"></i> <?= lang('create_purchases') ?>
              </a>
            </li>
            <li>
...
<script>
  $(document).ready(function() {
    ...
    $('#create_purchases').click(function () {
      createPurchases();
    });
      ...
      addConfirm({
        ...
        onok: function () {
          ...
          $.ajax({
            ...
            url: site.base_url + 'procurements/purchases/updateSafetyStock'
          });
        },
  ...
  function createPurchases () {
    ...
    for (let x in suppl) {
      if (suppl[x].checked) {
        suppl_ids += suppl[x].value + ',';
      }
    }

    let data = {};
    suppl_ids = suppl_ids.substr(0, suppl_ids.length - 1); // Remove last character ','.

    data[security.csrf_token_name] = security.csrf_hash;
    data.supplier_ids = suppl_ids; // String: "1,2,3,4,5,..."
    ...
    $.ajax({
      ...
      success: function (data) {
        console.log(data);
        if (typeof data == 'object' && ! data.error) {
          location.href = site.base_url + 'procurements/purchases';
        } else if (typeof data == 'object' && data.error) {
          addAlert(data.msg, 'danger');
        } else {
          addAlert('Unknown error', 'danger');
      ...
      url: site.base_url + 'procurements/purchases/addPurchasesFromPlan'
    });
  }
