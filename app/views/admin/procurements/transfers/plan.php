...
<script>
  ...
  function createTransfers () {
    ...
    let data = {};
    wh_ids = wh_ids.substr(0, wh_ids.length - 1); // Remove last character ','.

    data[security.csrf_token_name] = security.csrf_hash;
    data.warehouse_ids = wh_ids; // String: "1,2,3,4,5,..."
    ...
    $.ajax({
      ...
      success: function (data) {
        console.log(data);
        if (typeof data == 'object' && ! data.error) {
          location.href = site.base_url + 'procurements/transfers';
        } else if (typeof data == 'object' && data.error) {
          if (typeof data.msg == 'object') {
            addAlert(JSON.stringify(data.msg), 'danger');
      ...
      url: site.base_url + 'procurements/transfers/addTransfersFromPlan'
    });
  }
