...
<script>
  $(document).ready(function() {
    ...
    history.pushState(null, null, site.base_url + 'qms/counter');
  });
</script>
<?php endif; ?>
<link href="<?= $assets ?>qms/css/ridintek.css?<?= $res_hash ?>" rel="stylesheet" />
<div class="box">
  <div class="box-header">
    <h2 class="blue">
      <i class="fa-fw fad fa-user-headset"></i><?=lang('counter');?> <span id="counter_number"></span>
    </h2>

    <div class="box-icon">
      <ul class="btn-tasks">
        <li class="dropdown">
          <a href="#" id="holdMe" title="Filter"><i class="icon"></i></a>
        </li>
      </ul>
    </div>
  </div>
    ...
    <div class="row">
      ...
      <div class="col-md-12">
        <div class="panel panel-success">
          <div class="panel-heading">COUNTER INFORMATION</div>
          <div class="panel-body">
            <div class="col-sm-3 bold">Counter Status</div><div class="col-sm-3" id="counter_status"></div>
            <div class="col-sm-3 bold">Over-Waiting Call Time</div><div class="col-sm-3" id="timer_over_waitcall"></div>
            <div class="col-sm-3 bold">Progress Time</div><div class="col-sm-3" id="timer_progress"></div>
            <div class="col-sm-3 bold">Over-Waiting Serve Time</div><div class="col-sm-3" id="timer_over_waitserve"></div>
            <div class="col-sm-3 bold">Resting Time</div><div class="col-sm-3" id="timer_resting"></div>
            <div class="col-sm-3 bold">Over-Resting Time</div><div class="col-sm-3" id="timer_over_rest"></div>
            <div class="col-sm-3 bold">Serving Time</div><div class="col-sm-3" id="timer_serving"></div>
            <div class="col-sm-3 bold">Over-Serving Time</div><div class="col-sm-3" id="timer_over_serve"></div>
          </div>
        </div>
      </div>
      <div class="col-md-12">
        <div class="panel panel-info" style="margin-bottom: 0">
          ...
          <div class="panel-body">
            <div class="col-sm-3 bold">Ticket</div><div class="col-sm-3" id="cust_ticket">-</div>
            <div class="col-sm-3 bold">Time Limit</div><div class="col-sm-3" id="timer_cust_time_limit"></div>
            <div class="col-sm-3 bold">Category</div><div class="col-sm-3" id="cust_category">-</div>
            <div class="col-sm-3 bold">Waiting Time</div><div class="col-sm-3" id="timer_cust_waiting"></div>
            <div class="col-sm-3 bold">Name</div><div class="col-sm-3" id="cust_name">-</div>
            <div class="col-sm-3 bold">Serving Time</div><div class="col-sm-3" id="timer_cust_serving"></div>
            <div class="col-sm-3 bold">Phone</div><div class="col-sm-3" id="cust_phone">-</div>
            <div class="col-sm-3 bold">Over-Serving Time</div><div class="col-sm-3" id="timer_cust_over_serve"></div>
          </div>
        </div>
      </div>
    </div>
  ...
  $(document).ready(function () {
    ...
    btnCall.click(async function () { // CALL
      ...
      if (typeof window.recallTicketid !== 'undefined' && window.recallTicketid) {
        res = await QHttp.send('GET', site.base_url + 'qms/recallQueue/' + window.recallTicketid);
        delete(window.recallTicketid);
      } else {
        res = await QHttp.send('GET', site.base_url + 'qms/callQueue/' + warehouse_id);
      }
      ...
      if (old_ticket) { // END OLD TICKET
        ...
        if (Counter.status == 'serve' || Counter.status == 'paused') {
          QHttp.send('POST', site.base_url + 'qms/endQueue', data).then((r) => {
            if ( ! r.error) {
          ...
          QHttp.send('POST', site.base_url + 'qms/skipQueue', data).then((r) => {
            if ( ! r.error) {
      ...
      if ( ! res.error) {
        ...
        QConfig.set('slcustomer', ticket.customer_id); // For add sale.
        QConfig.set('slwarehouse', warehouse_id); // Fo add sale.
    ...
    btnRecall.click(async function () { // RECALL
      ...
      let res = await QHttp.send('GET', site.base_url + 'qms/recallQueue/' + ticket.id);
    ...
    btnServe.click(async function () { // START SERVING
      ...
      let res = await QHttp.send('POST', site.base_url + 'qms/serveQueue', data);
    ...
    btnExtend.click(function () {
      ...
      window.setTimeout(() => {$(this).prop('disabled', false);}, 10 * 1000); // Time out for 10 seconds.
    });

    btnAddSale.click(function () {
      location.href = site.base_url + 'sales/add?opt=noattachment';
    ...
    cbCounter.change(function () { // Counter number change.
      ...
      data[security.csrf_token_name] = security.csrf_hash;
      QHttp.send('POST', site.base_url + 'qms/setCounter', data).then((res) => {
        if ( ! res.error) {
