...
<script>
  $(document).ready(function() {
    ...
    window.Table = $('#MachineTable').DataTable({
      ajax: {
        data: function(data) {
          data[security.csrf_token_name] = security.csrf_hash;
        },
        method: 'POST',
        url: site.base_url + 'machines/maintenance/getLogs?<?= $q ?>'
      },
      columnDefs: [{
          targets: 0,
          orderable: false,
    ...
    <div class="row">
      <div class="col-lg-12">
        <div id="form_filter" class="closed well well-sm" style="display: none">
          <?php echo admin_form_open('machines'); ?>
          <div class="row">
            <div class="col-sm-4">
        ...
        <div class="row">
          <div class="col-sm-3 float-right">
            <div class="input-group">
              <input id="dtfilter" class="form-control dtfilter" data-name="machines" placeholder="<?= lang('search'); ?>">
        ...
        <table id="MachineTable" class="table table-bordered table-condensed table-hover table-striped" cellpadding="0" cellspacing="0" borders="0" style="width:100%">
          <thead>
            <tr>
...
<script>
  $(document).ready(function() {
    $('#btn_filter').click(function() {
      ...
      location.href = site.base_url + 'machines/maintenance/logs' + q;
    });
  });
</script>