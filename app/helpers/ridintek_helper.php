...
{
  $data['start_date'] = ($period['start_date'] ?? date('Y-m-') . '01');
      ...
      foreach ($saleItems as $saleItem) {
        if ($saleItem->product_type == 'combo') {
          ...
          foreach ($comboItems as $comboItem) {
            ...
            $soldItemCost += round($item->markon_price * ($comboItem->quantity * $saleItem->quantity));
          }
        }
      }
    }
...
function getPermission($permission_name, $user_id = NULL)
{
  ...
  if ($user) {
    ...
    } else if ($ci->session->userdata('permissions')) {
      $perms = $ci->session->userdata('permissions'); // Get permission by session.
    } else {
...
function setCreatedBy($data)
{
  ...
  if (!empty($data['created_by'])) {
    ...
    if ($creatorId = $ci->session->userdata('user_id')) {
...
{
  if ($msg && strlen($msg)) {
    $_SESSION['lastErrorMsg'] = $msg;
  } else {
    $_SESSION['lastErrorMsg'] = NULL;
  }

  if (isset($_SESSION['lastErrorMsg'])) {
    return TRUE;
  }
