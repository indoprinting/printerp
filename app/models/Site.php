...
class Site extends MY_Model
{
  ...
  public function addCustomers($data)
  {
    ...
    if (is_array($data)) {
      foreach ($data as $customerData) {
        $ids[] = $this->addCustomer($customerData);
      }
    }
    return $ids;
  }
  ...
  public function addExpensePayment($id, $status, $note)
  {
    ...
    if (!$expense) {
      setLastError('Expense is not found.');
      return FALSE;
    }
    ...
    if ($paymentId = $this->addPayment($payment)) {
      $expenseData = [
      ...
      if ($this->updateExpense($id, $expenseData)) {
        return $paymentId;
      } else {
  ...
  public function getUser($id = NULL)
  {
    ...
    if (!$q) {
      rd_print($this->db->error()['message']);
    }
    if ($q->num_rows() > 0) {
      return $q->row();
    }
  ...
  public function syncSales($clause = [])
  {
    ...
    if (!empty($clause['sale_id'])) {
      ...
      if ($saleType == 'array') {
        ...
        $sales[] = $this->getSaleByID($clause['sale_id']);
      } else {
    ...
    if ($paymentValidation) {
      foreach ($paymentValidation as $pv) {
        ...
        foreach ($dataMutasi as $dm) { // DM = Data Mutasi.
          ...
          // CR(mutasibank) = credit(cekmutasi) = Masuk ke rekening.
          // DB(mutasibank) = debet(cekmutasi) = Keluar dari rekening.
          if (
            ($amount_match && ($dm->type == 'CR' || $dm->type == 'credit')) ||
