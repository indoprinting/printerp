...
class Purchases_model extends CI_Model
{
  ...
  public function addPayment($data = [])
  {
    if ($this->db->insert('payments', $data)) {
      if ($this->site->getReference('ppay') == $data['reference']) {
        $this->site->updateReference('ppay');
      }
      $this->site->syncPurchasePayments($data['purchase_id']); // Override payment_status to need_approval
      return true;
    }
    return false;
  }
  ...
  public function addPurchase($data, $items) // NOT USED
  {
    $this->db->trans_start();
    if ($this->db->insert('purchases', $data)) {
      $purchase_id = $this->db->insert_id();
      if ($this->site->getReference('po') == $data['reference']) {
  ...
  {
    $this->db->select('purchase_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, products.unit, products.details as details, products.hsn_code as hsn_code, products.second_name as second_name')
  ...
  public function getPurchaseItemByID2($purchase_id, $product_id) // Added
  {
  ...
  public function returnPurchase($data = [], $items = [])
  {
    ...
    if ($this->db->insert('return_purchases', $data)) {
      ...
      if ($this->site->getReference('rep') == $data['reference']) {
        $this->site->updateReference('rep');
      }
      foreach ($items as $item) {
  ...
  public function updatePurchase ($id, $data, $items = []) // Partial not used here.
  {
    $this->db->trans_start();
    ...
    if ($this->db->update('purchases', $data, ['id' => $id])) {
      ...
      foreach ($items as $item) {
        $item['purchase_id'] = $id;
        $item['option_id']   = !empty($item['option_id']) && is_numeric($item['option_id']) ? $item['option_id'] : null;
        $this->db->update('purchase_items', $item, ['purchase_id' => $id, 'product_id' => $item['product_id']]);
        if ($data['status'] == 'received') {
          $this->updateAVCO(['product_id' => $item['product_id'], 'warehouse_id' => $item['warehouse_id'], 'quantity' => $item['quantity'], 'cost' => $item['real_unit_cost']]);
          $this->site->addProductIncreaseHistory([ // Add Product Increase History for Purchase.
      ...
      if ($data['status'] == 'paid' || $data['status'] == 'partial') {
        $this->site->syncPurchasePayments($id); // Mengubah payment_status ke "partial" atau "paid".
      }
    }
    $this->db->trans_complete();
    if ($this->db->trans_status() === false) {
  ...
  public function updatePaymentStatus($id, $status, $note)
  {
    $this->db->trans_start();
    $this->db->update('purchases', ['payment_status' => $status, 'note' => $note], ['id' => $id]);
    $this->db->trans_complete();
    if ($this->db->trans_status() === false) {
