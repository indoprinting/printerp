...
class Reports_model extends CI_Model
{
  ...
  {
    $this->db->from('sales')->where('customer_id', $customer_id)->where('status', 'returned');
    return $this->db->count_all_results();
  }
  ...
  {
    $myQuery = "SELECT DATE_FORMAT( date,  '%e' ) AS date, 0 AS tax1, 0 AS tax2, SUM( COALESCE( grand_total, 0 ) ) AS total, 0 AS discount, SUM( COALESCE( shipping, 0 ) ) AS shipping
  ...
  {
    $myQuery = "SELECT DATE_FORMAT( date,  '%e' ) AS date, '0' AS tax1, '0' AS tax2, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( discount, 0 ) ) AS discount, SUM( COALESCE( shipping, 0 ) ) AS shipping
  ...
  {
    $myQuery = "SELECT DATE_FORMAT( date,  '%c' ) AS date, SUM( COALESCE( product_tax, 0 ) ) AS tax1, SUM( COALESCE( order_tax, 0 ) ) AS tax2, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( discount, 0 ) ) AS discount, SUM( COALESCE( shipping, 0 ) ) AS shipping
      FROM " . $this->db->dbprefix('purchases') . ' WHERE ';
    if ($warehouse_id) {
      $myQuery .= " warehouse_id = {$warehouse_id} AND ";
    }
  ...
  {
    $myQuery = "SELECT DATE_FORMAT( date,  '%e' ) AS date, SUM( COALESCE( product_tax, 0 ) ) AS tax1, SUM( COALESCE( order_tax, 0 ) ) AS tax2, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( discount, 0 ) ) AS discount, SUM( COALESCE( shipping, 0 ) ) AS shipping
      FROM " . $this->db->dbprefix('purchases') . ' WHERE ';
    if ($warehouse_id) {
      $myQuery .= " warehouse_id = {$warehouse_id} AND ";
    }
