...
class Cron_model extends CI_Model
{
  ...
  public function run_daily()
  {
    if ($this->site->syncPaymentValidations()) {
      $this->rdlog->info('Payment validations have been synced successfully.');
    }

    if ($this->resetOrderRef()) { // OK
      $this->rdlog->info(lang('order_ref_updated'));
    }

    $date = date('Y-m-d H:i:s', strtotime('-1 month'));
    if ($this->deleteUserLogins($date)) {
      $this->rdlog->info(sprintf(lang('user_login_deleted'), $date));
    }

    if ($sess = $this->site->clearSessionStorage()) {
      $this->rdlog->info(sprintf('%d session have been deleted.', $sess));
    }

    if ($jobs = $this->site->clearOldWAJobs()) {
      $this->rdlog->info(sprintf('%d WA Jobs have been deleted.', $jobs));
    }
    ...
    if ($stats = $this->site->clearEmptyPayments()) {
      foreach ($stats as $stat) {
        $this->rdlog->info(sprintf("Payment for [%s id:%d] has been deleted.", $stat['type'], $stat['id']));
      }
    }
  ...
  protected function syncSafetyStock()
  {
    $m = [];
  ...
  private function resetOrderRef() // OK
  {
    if ($this->Settings->reference_format == 1 || $this->Settings->reference_format == 2) {
      $month = date('Y-m') . '-01';
      $year  = date('Y') . '-01-01';
      if ($ref = $this->getOrderRef()) {
        $reset_ref = ['adjustment' => 1, 'expense' => 1, 'income' => 1, 'iuse' => 1, 'mutation' => 1,
          'opname' => 1, 'purchase' => 1, 'sale' => 1, 'transfer' => 1];
        if ($this->Settings->reference_format == 1 && strtotime($ref->date) < strtotime($year)) {
          $reset_ref['date'] = $year;
          $this->db->update('order_ref', $reset_ref, ['ref_id' => 1]);
          return true;
        } elseif ($this->Settings->reference_format == 2 && strtotime($ref->date) < strtotime($month)) { // Current configuration.
