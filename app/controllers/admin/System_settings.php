...
class system_settings extends MY_Controller
{
  ...
  public function brands()
  {
    ...
    $this->page_construct('settings/brands', $this->data);
  }
  ...
  public function edit_income_category($id = null)
  {
    $this->form_validation->set_rules('code', lang('category_code'), 'trim|required');
    $category = $this->settings_model->getIncomeCategoryByID($id);
    if ($this->input->post('code') != $category->code) {
      $this->form_validation->set_rules('code', lang('category_code'), 'required|is_unique[income_categories.code]');
    }
    $this->form_validation->set_rules('name', lang('category_name'), 'required|min_length[3]');
    ...
    } elseif ($this->input->post('edit_income_category')) {
      $this->session->set_flashdata('error', validation_errors());
      admin_redirect('system_settings/income_categories');
    }

    if ($this->form_validation->run() == true && $this->settings_model->updateIncomeCategory($id, $data, $photo)) {
      $this->session->set_flashdata('message', lang('income_category_updated'));
      admin_redirect('system_settings/income_categories');
    } else {
      $this->data['error']    = validation_errors() ? validation_errors() : $this->session->flashdata('error');
      $this->data['category'] = $category;
      $this->load->view($this->theme . 'settings/edit_income_category', $this->data);
    }
  }
  ...
  public function income_category_actions()
  {
    $this->form_validation->set_rules('form_action', lang('form_action'), 'required');
    ...
    if ($this->form_validation->run() == true) {
      if (!empty($_POST['val'])) {
        if ($this->input->post('form_action') == 'delete') {
          foreach ($_POST['val'] as $id) {
            $this->settings_model->deleteIncomeCategory($id);
          }
          $this->session->set_flashdata('message', lang('categories_deleted'));
          redirect($_SERVER['HTTP_REFERER']);
        }
        ...
        if ($this->input->post('form_action') == 'export_excel') {
          ...
          $row = 2;
          foreach ($_POST['val'] as $id) {
            $sc = $this->settings_model->getIncomeCategoryByID($id);
            $this->excel->getActiveSheet()->SetCellValue('A' . $row, $sc->code);
            $this->excel->getActiveSheet()->SetCellValue('B' . $row, $sc->name);
            $row++;
          }
          ...
          $this->excel->getActiveSheet()->getColumnDimension('B')->setWidth(20);
          $this->excel->getDefaultStyle()->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
          $filename = 'income_categories_' . date('Y_m_d_H_i_s');
          $this->load->helper('excel');
          create_excel($this->excel, $filename);
        }
      } else {
  ...
  public function import_product_categories () // NOT USED
  {
    $this->load->helper('security');
    $this->form_validation->set_rules('userfile', lang('upload_file'), 'xss_clean');
      ...
      if (isset($_FILES['userfile'])) {
        ...
        foreach ($csvs as $csv) {
          $data_category = [
            ...
            'slug'        => $this->sma->slug($code),
            'image'       => 'no_image.png',
            'parent_id'   => NULL,
            'description' => rd_trim($csv['description'])
          ];
          if ($c = $this->settings_model->getCategoryByCode($data_category['code'])) {
            $updated .= '<p>' . lang('category_updated') . ' (' . $code . ')</p>';
            $this->settings_model->updateCategory($c->id, $data_category);
          } else {
    ...
    if ($this->form_validation->run() == true && $this->settings_model->addCategories($categories, $subcategories)) {
      ...
      $this->load->view($this->theme . 'settings/import_product_categories', $this->data);
    }
  }
    ...
    if ($this->form_validation->run() == true) {
      if (!empty($_POST['val'])) {
        if ($this->input->post('form_action') == 'update_price') {
          foreach ($_POST['val'] as $id) {
            $this->settings_model->setProductPriceForPriceGroup($id, $group_id,
              $this->input->post('price' . $id),
              $this->input->post('price2_' . $id),
              $this->input->post('price3_' . $id),
              $this->input->post('price4_' . $id),
              $this->input->post('price5_' . $id),
              $this->input->post('price6_' . $id)
            );
          }
  ...
  public function testmail () {
    ...
    if (empty($to)) {
      sendJSON(['error' => 1, 'msg' => 'No recipient email']);
    }
    $body = "<html><head></head>";
    $body .= "<body><b>Mail Test</b> <i>From</i> <u>indoprinting.co.id</u></body>";
    $body .= "</html>";
    if ($this->sma->send_email($to, 'Test Subject', $body, $this->Settings->smtp_user) !== FALSE) {
      sendJSON(['error' => 0, 'msg' => 'Email sent successfully.']);
    }
    sendJSON(['error' => 1, 'msg' => $this->session->userdata('error')]);
  }
